-- Fix "column reference created_at is ambiguous" in get_semantic_feed.
--
-- get_semantic_feed is a plpgsql function whose returns-table declaration
-- includes `created_at timestamptz`. In plpgsql, that creates an implicit
-- OUT variable named `created_at`. Any unqualified `created_at` inside the
-- function body is then ambiguous between the variable and a table column,
-- raising PG error 42702. The fix is to qualify every reference with a
-- table alias.
