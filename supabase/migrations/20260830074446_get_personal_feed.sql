-- get_personal_feed: core ranking RPC for the "For You" feed.
--
-- SECURITY: matches the fix applied to get_dm_conversations in 20260830043429.
-- The brief for this function had it personalize for whatever p_user_id it was
-- handed. That is an IDOR: the function is SECURITY DEFINER with EXECUTE
-- granted to authenticated, so any logged-in caller could pass someone else's
-- uuid and receive that person's personalized feed (candidate set, taste-based
-- ranking, affinity boosts) — learning what content that person is shown.
-- Identity is instead derived from the JWT via v_uid and used everywhere the
-- body would otherwise reference p_user_id. p_user_id is kept only for
-- signature compatibility and as an advisory fallback for service_role, which
-- has no auth.uid() of its own. refresh_user_taste (20260830072647 / its
-- follow-up fix) already rejects a mismatched uid for non-service_role
-- callers, so passing v_uid everywhere is required for that call to succeed
-- for a normal authenticated user.
--
-- pgvector lives in extensions, and this function uses extensions.vector(768)
-- and the <=> operator, so search_path MUST include extensions or vector
-- fails to resolve at runtime (42704) — the same mistake that disabled four
-- RPCs in this codebase for 19 days.
--
-- Every reader of public_echoes must filter check_content = true; RLS does
-- not enforce it (qual: true) and SECURITY DEFINER bypasses RLS anyway.
