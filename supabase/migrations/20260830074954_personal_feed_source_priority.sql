-- Fix round 1 on get_personal_feed (introduced 20260830074446):
-- the candidates CTE picked source via min(c.source), which is an
-- alphabetical tie-break, not a priority order. 'exploration' sorts before
-- 'follow', 'semantic' and 'trending', so any echo that also happened to
-- land in the random exploration sample got permanently mislabelled
-- 'exploration' even when it was a genuine follow/semantic/trending hit.
--
-- This is load-bearing, not cosmetic: Task 6's applyDiversity() reserves
-- ~20% of every page for source = 'exploration'. With the alphabetical
-- tie-break, real follow/semantic/trending hits masquerade as exploration
-- and the reserve stops reserving anything -- the diversity guarantee
-- silently stops holding.
--
-- Fix: replace min(c.source) with a priority-ordered pick (follow >
-- semantic > trending > exploration) so the most informative source wins
-- whenever an echo qualifies through more than one path. Everything else
-- in the function, including the v_uid identity resolution from the IDOR
-- fix, is unchanged.
