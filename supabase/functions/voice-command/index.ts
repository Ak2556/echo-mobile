...
- "open_mini_app": open a specific tool/mini-app. args.app MUST be one canonical id: pomodoro (timer/focus/टाइमर), tasks (todo/टास्क/काम), notes (नोट), habits (आदत), expenses (money/budget/पैसा/खर्च/बजट), fitness (workout/gym/कसरत/व्यायाम), shopping-list (shopping/खरीदारी/सामान), calculator (calc/हिसाब/गणना), learn (study/course/सीखो/पढ़ाई), planner (schedule/योजना), voice-memo (memo/record/रिकॉर्ड/आवाज़ नोट), camera (photo/कैमरा/फोटो), image-editor (photo editor/फोटो एडिटर), world-clock (clock/timezone/घड़ी/समय क्षेत्र), markdown (write/लिखो/लेखन), password-gen (password/पासवर्ड). Optionally set args.action (+ args.value) to ENTER DATA inside the app — pomodoro: action=start|stop; tasks/notes/habits/shopping-list/planner: action=add, value=the item text (keep the user's language); expenses: action=add, value=amount + category, e.g. "500 food" or "2000 salary" (खर्च जोड़ो 500 खाना → app=expenses,action=add,value="500 food"); fitness: action=add, value="weight 70" to log weight or "water" to log a glass of water. Examples: "पोमोडोरो खोलो"→app=pomodoro, "टाइमर चालू करो"→app=pomodoro,action=start, "टाइमर बंद करो"→app=pomodoro,action=stop, "टास्क जोड़ो दूध लाना"→app=tasks,action=add,value="दूध लाना", "add task call mom"→app=tasks,action=add,value="call mom", "नोट लिखो idea"→app=notes,action=add,value="idea", "आदत जोड़ो पानी पीना"→app=habits,action=add,value="पानी पीना".
