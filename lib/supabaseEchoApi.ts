...
async function signedDmMediaUrl(value: string | null | undefined): Promise<string | null> {
  ...
  return `${process.env.EXPO_PUBLIC_CLOUDFLARE_WORKER_URL}/dm-media/${path}`;
}
...
export async function uploadAvatar(image: UploadableImage): Promise<string> {
  ...
  const workerRes = await fetch(`${process.env.EXPO_PUBLIC_CLOUDFLARE_WORKER_URL}/upload-url?bucket=avatars&path=${path}`, {
...
export async function uploadEchoImages(images: UploadableImage[]): Promise<string[]> {
  ...
  for (let i = 0; i < Math.min(images.length, 4); i++) {
    ...
    const workerRes = await fetch(`${process.env.EXPO_PUBLIC_CLOUDFLARE_WORKER_URL}/upload-url?bucket=echo-media&path=${path}`, {
      headers: { 'Authorization': `Bearer ${session.access_token}` }
    });
    const { signedUrl, publicUrl } = await workerRes.json();
...
const PROFILE_SELECT = 'id, username, display_name, bio, avatar_color, avatar_url, is_verified, created_at, follower_count, mood, mood_expires_at, pronouns, pinned_echo_id, is_moderator, is_private, dm_privacy, activity_status, online_status, read_receipts, sensitive_content_filter, personalized_notifications';
