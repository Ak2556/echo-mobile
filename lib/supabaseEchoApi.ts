...
export const CO_AUTHOR_CONSENT_REQUIRED_MESSAGE = 'Co-author posting is temporarily unavailable until co-author approval is required.';
...
export async function insertRemoteEcho(params: {
  ...
  if (params.coAuthorId || params.coAuthorResponse?.trim()) {
    throw new Error(CO_AUTHOR_CONSENT_REQUIRED_MESSAGE);
  }
  const title =
    params.title?.trim() ||
