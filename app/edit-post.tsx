...
export default function EditPostScreen() {
  ...
  const [imageUrls, setImageUrls] = useState<string[]>(echo?.mediaUris?.length ? echo.mediaUris : ['']);
  const [videoUrl, setVideoUrl] = useState(echo?.videoUri ?? '');
  const [pollQuestion, setPollQuestion] = useState(echo?.poll?.question ?? '');
  const [pollOptions, setPollOptions] = useState<string[]>(echo?.poll?.options.map(o => o.text) ?? ['', '']);
  ...
  const handleSave = () => {
    ...
    switch (postType) {
      case 'text':
        ...
        break;
      case 'photo':
        Object.assign(updates, { prompt: caption.trim() || 'Photo post', response: '', mediaUris: validImages, videoUri: undefined, poll: undefined });
        break;
      case 'video':
        Object.assign(updates, { prompt: caption.trim() || 'Video post', response: '', videoUri: videoUrl.trim(), mediaUris: undefined, poll: undefined });
        break;
      case 'poll': {
        const options: PollOption[] = pollOptions.filter(o => o.trim()).map((o, i) => {
          const existing = echo.poll?.options[i];
          return { id: existing?.id ?? `opt_${i}`, text: o.trim(), votes: existing?.votes ?? 0 };
        });
        const endsAt = echo.poll?.endsAt ?? new Date(Date.now() + pollDurationHours * 3600000).toISOString();
        Object.assign(updates, { prompt: pollQuestion.trim(), response: '', mediaUris: undefined, videoUri: undefined, poll: { question: pollQuestion.trim(), options, totalVotes: echo.poll?.totalVotes ?? 0, userVote: echo.poll?.userVote, endsAt } });
        break;
      }
    }
            ...
            <Animated.View entering={animation(FadeIn.duration(180))}>
              ...
              {imageUrls.map((url, idx) => (
                <View key={idx} style={[s.surface, { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 4, marginBottom: 8 }]}>
                  <TextInput value={url} onChangeText={v => updateImageUrl(idx, v)} placeholder={`https://... (image ${idx + 1})`} placeholderTextColor={colors.textMuted} autoCapitalize="none" autoCorrect={false} style={{ flex: 1, color: colors.text, fontSize: fontSizes.small, paddingVertical: 12 }} />
                  {imageUrls.length > 1 && <Pressable onPress={() => removeImageUrl(idx)} style={{ padding: 4 }}><X color={colors.textMuted} size={16} /></Pressable>}
                </View>
              ))}
            ...
            <Animated.View entering={animation(FadeIn.duration(180))}>
              ...
              {pollOptions.map((opt, idx) => (
                <View key={idx} style={[s.surface, { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 14, paddingVertical: 2, marginBottom: 8 }]}>
                  ...
                  {pollOptions.length > 2 && <Pressable onPress={() => removePollOption(idx)} style={{ padding: 4 }}><X color={colors.textMuted} size={16} /></Pressable>}
                </View>
              ))}
