...
const PRIORITIES: { id: TaskPriority; label: string; tone: 'textSecondary' | 'warning' | 'danger' | 'success' }[] = [
  { id: 'normal', label: 'Normal', tone: 'textSecondary' },
    ...
    <View style={{ flex: 1, backgroundColor: colors.bg }}>
      <MiniAppShell title={tt('Tasks')} subtitle={tt('Action')}>
        ...
        <View style={{ gap: 10 }}>
          {visible.map(task => {
            ...
            const pColor = colors[priorityMeta.tone];
            return (
              <Animated.View key={task.id} entering={FadeIn} exiting={FadeOut} layout={Layout.springify()}>
                <GlassPanel variant="light" borderRadius={radius.xl} contentStyle={{ padding: 14 }}>
                  <Pressable onPress={() => openDetail(task)} style={{ flexDirection: 'row', alignItems: 'center', gap: 12 }}>
                    ...
                    <View style={{ flex: 1, minWidth: 0 }}>
                      ...
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 5, marginTop: 4, flexWrap: 'wrap' }}>
                        ...
                        {task.priority === 'urgent' && <WarningCircle color={pColor} size={12} weight="bold" />}
                        <Text style={{ color: task.priority === 'urgent' ? pColor : colors.textMuted, fontSize: 12, fontWeight: task.priority === 'urgent' ? '700' : '500' }}>
                    ...
                    <Flag color={pColor} size={17} weight={task.priority === 'low' ? 'regular' : 'fill'} />
        ...
        <Modal visible={!!detailTask} animationType="slide" transparent={true} onRequestClose={closeDetail}>
          {detailTask && (
            <View style={{ flex: 1, backgroundColor: colors.isDark ? 'rgba(0,0,0,0.75)' : 'rgba(0,0,0,0.45)', justifyContent: 'flex-end' }}>
              <Pressable style={{ flex: 1 }} onPress={closeDetail} />
              <Animated.View entering={SlideInDown} exiting={SlideOutDown} style={{ backgroundColor: colors.bg, borderTopLeftRadius: radius.xl, borderTopRightRadius: radius.xl, padding: 24, paddingBottom: Platform.OS === 'ios' ? 48 : 24, maxHeight: '90%' }}>
                <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 }}>
                  ...
                  <Pressable onPress={closeDetail} style={{ backgroundColor: colors.surfaceHover, padding: 8, borderRadius: radius.card }}>
                ...
                <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ gap: 24 }}>
                  ...
                  <View>
                    ...
                    <TextInput
                      ...
                      style={{ backgroundColor: colors.surfaceHover, color: colors.text, padding: 16, borderRadius: radius.card, fontSize: 15, minHeight: 80 }}
                      multiline
                    />
                  </View>
                  ...
                  <View>
                    ...
                    <View style={{ flexDirection: 'row', gap: 8, flexWrap: 'wrap' }}>
                      {PRIORITIES.map(p => {
                        const pColor = colors[p.tone];
                        return (
                          <Pressable key={p.id} onPress={() => updateTask(detailTask.id, { priority: p.id })}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: detailTask.priority === p.id ? pColor : colors.surfaceHover, paddingHorizontal: 12, paddingVertical: 8, borderRadius: radius.md }}>
                              <Flag color={detailTask.priority === p.id ? colors.bg : pColor} size={14} weight="fill" />
                              <Text style={{ color: detailTask.priority === p.id ? colors.bg : colors.textSecondary, fontSize: 13, fontWeight: '700' }}>{p.label}</Text>
                            </View>
                          </Pressable>
                        );
                      })}
                  ...
                  <View>
                    ...
                    <View style={{ backgroundColor: colors.surfaceHover, borderRadius: radius.card, padding: 16 }}>
                      <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }}>
                        ...
                        <View style={{ flexDirection: 'row', gap: 8 }}>
                          {detailAlarm && (
                            <Pressable onPress={() => { setDetailAlarm(null); setShowAlarmPicker(false); updateTask(detailTask.id, { due: undefined }); }} style={{ padding: 6, backgroundColor: colors.surface, borderRadius: radius.sm }}>
                          ...
                          }} style={{ paddingHorizontal: 12, paddingVertical: 6, backgroundColor: detailAlarm ? accent : colors.surface, borderRadius: radius.sm }}>
                            <Text style={{ color: detailAlarm ? colors.bg : colors.text, fontWeight: '700' }}>
                  ...
                  <View>
                    ...
                    <View style={{ gap: 8 }}>
                      {detailTask.subtasks?.map(st => (
                        <Animated.View key={st.id} entering={FadeIn} layout={Layout.springify()} style={{ flexDirection: 'row', alignItems: 'center', gap: 12, backgroundColor: colors.surfaceHover, padding: 12, borderRadius: radius.md }}>
                      ...
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, marginTop: 4 }}>
                        <TextInput
                          ...
                          placeholderTextColor={colors.textMuted}
                          style={{ flex: 1, backgroundColor: colors.surfaceHover, color: colors.text, paddingHorizontal: 16, paddingVertical: 12, borderRadius: radius.md, fontSize: 15 }}
                          onSubmitEditing={addSubtask}
                        />
                        <Pressable onPress={addSubtask} style={{ backgroundColor: accent, padding: 12, borderRadius: radius.md }}>
                          <Plus color={colors.bg} size={16} weight="bold" />
                        </Pressable>
                      </View>
                    </View>
                  </View>
                  ...
                  <View>
                    ...
                    <TextInput
                      ...
                      placeholderTextColor={colors.textMuted}
                      style={{ backgroundColor: colors.surfaceHover, color: colors.text, padding: 16, borderRadius: radius.card, fontSize: 15 }}
                    />
                  </View>
        ...
        <Modal visible={showAlarmPicker} animationType="fade" transparent={true} onRequestClose={() => setShowAlarmPicker(false)}>
          <View style={{ flex: 1, backgroundColor: colors.isDark ? 'rgba(0,0,0,0.75)' : 'rgba(0,0,0,0.45)', justifyContent: 'flex-end' }}>
            ...
            <Animated.View entering={SlideInDown.duration(300).springify()} exiting={SlideOutDown.duration(200)} style={{ backgroundColor: colors.bg, borderTopLeftRadius: radius.xl, borderTopRightRadius: radius.xl, padding: 24, paddingBottom: Platform.OS === 'ios' ? 48 : 24 }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
                <Text style={{ fontSize: 22, fontWeight: '800', color: colors.text }}>Set Alarm</Text>
                <Pressable onPress={() => setShowAlarmPicker(false)} style={{ backgroundColor: colors.surfaceHover, padding: 8, borderRadius: radius.card }}>
                  <X color={colors.text} size={20} weight="bold" />
                </Pressable>
              </View>
              
              <View style={{ backgroundColor: colors.surface, borderRadius: radius.xl, padding: 16, marginBottom: 24, borderWidth: 1, borderColor: colors.glassBorder }}>
              ...
              >
                <Text style={{ color: colors.bg, fontSize: 18, fontWeight: '700' }}>Save Alarm</Text>
              </Pressable>
            </Animated.View>
          </View>
        </Modal>
      ...
      {showAddSheet && (
        <View style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 999 }}>
          <Pressable style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: colors.isDark ? 'rgba(0,0,0,0.75)' : 'rgba(0,0,0,0.45)' }} onPress={() => setShowAddSheet(false)} />
          <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, justifyContent: 'flex-end' }}>
            <Animated.View entering={SlideInDown.springify().damping(18).stiffness(200)} exiting={SlideOutDown} style={{ backgroundColor: colors.surface, borderTopLeftRadius: radius.xl, borderTopRightRadius: radius.xl, padding: 24, paddingBottom: 40, maxHeight: '90%' }}>
              <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 20 }}>
            
            <View style={{ width: 40, height: 4, borderRadius: radius.sm, backgroundColor: colors.border, alignSelf: 'center', marginBottom: 20 }} />
            ...
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginBottom: 24 }}>
              {PRIORITIES.map(item => {
                const pColor = colors[item.tone];
                return (
                  <Pressable key={item.id} onPress={() => setPriority(item.id)}>
                    <View style={{ borderRadius: radius.full, paddingHorizontal: 16, paddingVertical: 10, backgroundColor: priority === item.id ? pColor : colors.surfaceHover, borderWidth: priority === item.id ? 0 : 1, borderColor: colors.border }}>
                      <Text style={{ color: priority === item.id ? colors.bg : colors.textSecondary, fontSize: 14, fontWeight: '800' }}>{tt(item.label)}</Text>
                    </View>
                  </Pressable>
                );
              })}
            ...
            <View style={{ flexDirection: 'row', gap: 8, marginBottom: showAddPicker ? 16 : 32 }}>
              <Pressable onPress={() => {
                ...
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8, borderRadius: radius.full, paddingHorizontal: 16, paddingVertical: 10, backgroundColor: alarmTime ? accent : colors.surfaceHover, borderWidth: alarmTime ? 0 : 1, borderColor: colors.border }}>
                  <Bell color={alarmTime ? colors.bg : colors.textSecondary} size={16} weight={alarmTime ? "fill" : "regular"} />
                  <Text style={{ color: alarmTime ? colors.bg : colors.textSecondary, fontSize: 14, fontWeight: '800' }}>
              ...
              {alarmTime && (
                <Pressable onPress={() => { setAlarmTime(null); setShowAddPicker(false); }}>
                  <View style={{ borderRadius: radius.full, paddingHorizontal: 16, paddingVertical: 10, backgroundColor: colors.surfaceHover, borderWidth: 1, borderColor: colors.border }}>
            ...
            <Pressable onPress={() => { add(); setShowAddSheet(false); }}>
              <View style={{ height: 56, borderRadius: radius.xl, backgroundColor: accent, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', gap: 10, elevation: 5 }}>
                <Plus color={colors.bg} size={20} weight="bold" />
                <Text style={{ color: colors.bg, fontSize: 17, fontWeight: '900' }}>{tt('Create Task')}</Text>
              </View>
            </Pressable>
