...
export default function ExpensesApp() {
  ...
  return (
    <MiniAppShell title="Expenses" subtitle="Income & budget log" headerRight={AddBtn}>
      {/* Balance card */}
      ...
      <GlassPanel variant="light" borderRadius={14} contentStyle={{ flexDirection: 'row', padding: 4 }} style={{ marginBottom: 14 }}>
        {(['all', 'income', 'expense'] as const).map(f => (
          <Pressable key={f} onPress={() => setFilter(f)} style={{ flex: 1, paddingVertical: 10, borderRadius: 10, alignItems: 'center', backgroundColor: filter === f ? (f === 'income' ? '#10B981' : f === 'expense' ? '#EF4444' : accent) : 'transparent' }}>
