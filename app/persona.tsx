...
export default function PersonaScreen() {
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: colors.bg }} edges={['top']}>
      <ScreenHeader
        title={ttx("Personal Persona")}
        subtitle={ttx("Echo learns how you think, write, and decide.")}
      />
      ...
      >
            ...
            <View style={styles.heroRow}>
              ...
              <View style={{ flex: 1, minWidth: 0 }}>
                ...
                <Text style={{ color: colors.textMuted, fontSize: fontSizes.caption, marginTop: 4, lineHeight: 19 }}>
                  {ttx("Persona becomes reliable after about 6 days and enough real conversations.")}
                </Text>
              </View>
              <Switch
            ...
            <View>
              <View style={styles.progressMeta}>
                <Text style={{ color: colors.textSecondary, fontSize: fontSizes.caption, fontWeight: '700' }}>
                  {progress}{ttx("% calibrated")}
                </Text>
                <Text style={{ color: colors.textMuted, fontSize: fontSizes.caption }}>
                  {status.daysObserved}{ttx("d ·")} {status.signalCount} {ttx("signals")}
                </Text>
              </View>
        ...
        <Animated.View entering={animation(FadeInDown.delay(80).duration(220))}>
          <GlassPanel borderRadius={radius.card} contentStyle={{ padding: 16, gap: 14 }}>
            <View style={styles.row}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: colors.text, fontSize: fontSizes.body, fontWeight: '700' }}>{ttx("First-week learning")}</Text>
                <Text style={{ color: colors.textMuted, fontSize: fontSizes.caption, marginTop: 3, lineHeight: 19 }}>
                  {ttx("Echo uses your own chats to learn recurring topics, voice, values, and answer style.")}
                </Text>
              </View>
            </View>
            <View style={styles.row}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={{ color: colors.text, fontSize: fontSizes.body, fontWeight: '700' }}>{ttx("Private by default")}</Text>
                <Text style={{ color: colors.textMuted, fontSize: fontSizes.caption, marginTop: 3, lineHeight: 19 }}>
                  {ttx("The persona summary is used only to personalize Echo replies when you message Echo.")}
                </Text>
              </View>
            </View>
          </GlassPanel>
        ...
        <Animated.View entering={animation(FadeInDown.delay(120).duration(220))}>
          <GlassPanel borderRadius={radius.card} contentStyle={{ padding: 16, gap: 16 }}>
            <SummaryList title={ttx("Voice and preferences")} items={profile.traits} empty="Chat naturally with Echo and this will fill in." />
            <SummaryList title={ttx("Values")} items={profile.values} empty="Say what matters to you and Echo will preserve it here." />
            <SummaryList title={ttx("Recurring topics")} items={profile.topics} empty="Topics appear as Echo sees repeated patterns." />
            <SummaryList title={ttx("Response style")} items={profile.responseStyle} empty="Echo will infer whether you prefer concise, deep, tactical, or reflective replies." />
          </GlassPanel>
        </Animated.View>
        ...
        <Animated.View entering={animation(FadeInDown.delay(160).duration(220))}>
          <GlassPanel borderRadius={radius.card} contentStyle={{ padding: 16, gap: 12 }}>
            <Text style={{ color: colors.text, fontSize: fontSizes.body, fontWeight: '700' }}>{ttx("Direct note")}</Text>
            <Text style={{ color: colors.textMuted, fontSize: fontSizes.caption, lineHeight: 19 }}>
              {ttx("Add anything Echo should always remember about your voice or judgment.")}
            </Text>
            <TextInput
              value={note}
              onChangeText={setNote}
              placeholder={ttx("Example: I like clear tradeoffs, direct language, and practical next steps.")}
        ...
        <View style={styles.actions}>
          ...
          >
            ...
            <Text style={{ color: colors.text, fontSize: fontSizes.body, fontWeight: '700' }}>{ttx("Chat with Echo")}</Text>
          </AnimatedPressable>
          <AnimatedPressable
            onPress={reset}
          ...
          >
            ...
            <Text style={{ color: colors.danger, fontSize: fontSizes.body, fontWeight: '700' }}>{ttx("Reset persona")}</Text>
          </AnimatedPressable>
        </View>
      </ScrollView>
    </SafeAreaView>
